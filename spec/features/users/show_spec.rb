  ...
  context 'viewing parties' do
    ...
    it 'has a section which shows user viewing parties', :vcr do
      ...
      within("#vp_#{@vp_1.id}") do
        ...
        expect(page).to have_link(@vp_1.movie_title)
        expect(page.find("#movie_img_#{@vp_1.movie_id}")['src']).to have_content(@vp_1.image_path)
      ...
      within("#vp_#{@vp_2.id}") do
        ...
        expect(page).to have_link(@vp_2.movie_title)
        expect(page.find("#movie_img_#{@vp_2.movie_id}")['src']).to have_content(@vp_2.image_path)
